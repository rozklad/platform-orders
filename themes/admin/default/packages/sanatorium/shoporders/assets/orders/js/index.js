...
;(function(window, document, $, undefined)
{
	...
	Extension.Index.datePicker = function()
	{
		...
		$('.daterangepicker_start_input').attr('data-grid', 'order');

		$('.daterangepicker_end_input').attr('data-grid', 'order');
	...
	Extension.Index.dataGrid = function()
	{
		var config = {
			...
			callback: function()
			{
		        ...
		        $('[data-send]').click(function(event){
		        	event.preventDefault();
		        	var id = $(this).data('id'),
		        ...
		        $('[data-forgot]').click(function(event){
		        	event.preventDefault();
		        	var id = $(this).data('id'),
		        		route = $(this).data('route'),
		        		success = $(this).data('msg-success');
		...
		Extension.Index.Grid = $.datagrid('order', '#data-grid', '#data-grid_pagination', '#data-grid_applied', config);

		return this;
	};

	$(window).on('custom_filters', function(){
		Extension.Index.Grid.refresh();
	});
